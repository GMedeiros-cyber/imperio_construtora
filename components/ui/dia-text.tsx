 ...
 *
 * - O componentThemeClassName foi REMOVIDO por inteiro. Eram dezenas de
 *   variáveis CSS de outro design system (--ic-*, --color-*) que
 *   sobrescreveriam os nossos tokens no elemento e em todos os filhos.
 * - textColor passa a cair em "currentColor" por padrão, e não na variável
 *   --ic-foreground que deixou de existir junto com o bloco acima.
 * - Respeita prefers-reduced-motion: com a preferência ativa o texto sai
 *   chapado, sem varredura, sem gradiente e sem motion value nenhum.
 */
...
const DiaTextReveal = forwardRef<HTMLSpanElement, DiaTextRevealProps>(
  (
    ...
    const isVisible = (triggerOnView ? inView : true) && !reduzido;
    ...
    /* Movimento reduzido: texto chapado, sem varredura nem gradiente. */
    if (reduzido) {
      return (
        <span ref={setRefs} className={cn("align-bottom leading-[100%]", className)}>
          {texts[activeIndex]}
        </span>
      );
    }
